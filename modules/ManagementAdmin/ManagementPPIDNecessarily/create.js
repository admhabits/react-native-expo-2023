...
const ManagementAdminNewsAndBroadcast = ({ navigation, route }) => {
  ...
  const handleSubmit = async () => {
    ...
    await axios
      .post(BaseUrl + '/api/ppid-serta-merta/create', data, {
        headers: {
      ...
      .then(res => {
        ...
        navigation.navigate('ManagementPPIDNecessarily')
      })
      .catch(err => {
  ...
  const handleUpdate = async () => {
    ...
    const token = login.data.token;
    await axios
      .post(`${BaseUrl}/api/ppid-serta-merta/update`, data, {
        headers: {
          Authorization: 'Bearer ' + token,
        },
      })
  ...
  return (
    <BaseContainerManagementAdmin
      title={'Tambah Informasi Publik Serta Merta'}
      namePathBack={'Manage PPID'}
      onPressBack={() => navigation.navigate('ManagementPPIDNecessarily')}>
      <View style={{ marginHorizontal: 15 }}>
        ...
        <View style={styles.actionButton}>
          ...
          <TouchableOpacity
            ...
            onPress={() => navigation.navigate('ManagementPPIDNecessarily')}>
