...
import { getPPIDPeriodically } from '../../../redux/ppidPeriodically/ppidPeriodically.actions';

const ManagementPeriodically = ({ navigation }) => {
  const { login, periodicallyPPID } = useSelector(state => state);
  ...
  useEffect(() => {
    setDataTable(periodicallyPPID?.data?.listData);
    // console.log('ini data periodicallyPPID', periodicallyPPID?.data?.listData);
  }, [periodicallyPPID?.data?.listData]);
  ...
  useEffect(() => {
    ...
    let filter = periodicallyPPID?.data?.listData?.filter(item =>
  ...
  const output = value => {
    if (value === 'SUCCESS') {
      ...
      //   position: 'center',
      // });
      dispatch(getPPIDPeriodically(token));
    } else {
      // RNToasty.Error({
    ...
    <>
      <View style={{ width: '100%', height: '100%', backgroundColor: '#fff' }}>
        ...
        <View style={styles.title}>
          <Text>Daftar Informasi Publik Berkala</Text>
          <TouchableOpacity
            onPress={() =>
              navigation.navigate('CreateManagementPPIDPeriodically')
            }
            style={{
              ...styles.button,
            ...
            <View key={i}>
              <View
                ...
                <View
                  ...
                  <TouchableOpacity
                    onPress={() => {
                      const { postTitle, author, postStatus, tanggal, postId, postExcerpt, fileName } = item;
                      return navigation.navigate('CreateManagementPPIDPeriodically', { postTitle, author, postStatus, tanggal, postId, postExcerpt, fileName })
                    }
                    }
        ...
        <ModalDelete
          title={'Apakah anda yakin ingin menghapus data PPID berkala?'}
          confirm={'Iya'}
          cancle={'Tidak'}
          isActive={isModalShowDelete}
          onCancelPressed={() => onCancelPressed()}
          path={`/api/ppid-berkala/delete?postId=${isParamDelete.postId}`}
          output={output}
        />
      </View>
    </>
...
export default ManagementPeriodically;
