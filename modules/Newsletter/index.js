...
const NewsLetter = ({ navigation }) => {
  ...
  const getData = () => {
    ...
    axios.get(BaseUrl + '/api/public/newsletter?search=&limit=100&offset=0')
    .then(res => {
      console.log('newsletter', data);
      if (res.data.data.letterResponses.length > 0) {
        setData(res.data.data.letterResponses);
        setDataNull('notnull');
      } else {
        setData([]);
        setDataNull('null');
          ...
          <View
            ...
            {isFilterData.map((item, i) => {
              ...
              <CardListGallery
                ...
                date={item.tanggal}
                type={'pdf'}
                assetsPath={'storage/documents/post'}
