...
const GaleriFoto = ({navigation}) => {
  ...
  const getData = () => {
    ...
    try {
      const get = fetch(BaseUrl + '/api/public/photo?limit=10&offset=0', {
        method: 'GET',
        headers: {
        ...
        .then(data => {
          if (data.data.galeriFotoResponses.length > 0) {
            setData(data.data.galeriFotoResponses);
            setDataNull('notnull');
          } else {
            setData([]);
            setDataNull('null');
          ...
          <View
            ...
            {isFilterData.map((item, i) => (
              <CardListGallery
                ...
                title={item.title}
                date={item.tanggal}
                assetsPath={'storage/portal/images/galleries/photo'}
